...
type Handler = (
  req: Request,
  res: Response,
  next: (errorMessage?: string) => unknown
) => unknown;
const holders: {
  user?: (...handlers: Handler[]) => unknown;
  admin?: (
    ...handlers: [{ permission: string }, ...Handler[]] | Handler[]
  ) => unknown;
...
const auth = (config = gConfig) => {
    ...
    try {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (await gConfig?.customValidator?.(req, key)) {
          return await handlerState[key].handlers[0](
            req,
            res,
            nextFunc(handlerState[key].handlers)
          );
  ...
  holders.admin = (
    ...
    const hasConfig =
      typeof handlers[0] === "object" && "permission" in handlers[0];
    const configObj = hasConfig
      ? (handlers[0] as { permission: string })
      : undefined;
    const handlerFns = hasConfig
      ? ((handlers as any[]).slice(1) as Handler[])
      : (handlers as Handler[]);
    handlerState[
      `admin${configObj?.permission ? `:${configObj.permission}` : ""}`
    ] = {
